...
| Selector and hierarchical reference coverage                       | Analysis + LSP               | Shipped, bounded | `!element` selector targets and dotted identifiers such as `system.api` now resolve for hover, definition, and references; completion and rename remain deferred. |
| Named dynamic relationship reference coverage                      | Analysis + LSP               | Shipped, bounded | Named dynamic relationship reference sites now resolve for hover, definition, and references within the current bounded symbol families.                          |
| Remaining scope/reference coverage (`this`, edge cases)            | Analysis + LSP               | Next             | The biggest remaining gap is the still-deferred scope edge cases outside the shipped bounded selector / dotted-reference / named-dynamic slice.                   |
...
| Workspace symbols                                                  | Analysis + LSP               | Shipped, bounded | `workspace/symbol` is already backed by workspace facts today; broader breadth and ranking still depend on widening workspace symbol fact coverage.               |
...
- richer read-only semantic feedback such as broader hover coverage and stronger workspace-symbol breadth/ranking
