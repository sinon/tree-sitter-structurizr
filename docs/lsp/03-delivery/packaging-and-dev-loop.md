...
This note defines the recommended packaging model and contributor loops for the future Structurizr DSL grammar, analysis layer, LSP, and Zed extension.
...
2. the future analysis crate
3. the future LSP binary
...
1. keep the grammar and future Rust crates in this repository
2. treat the analysis crate as an internal source-level workspace member, not a separately distributed editor artifact
3. distribute `structurizr-lsp` as pinned GitHub release binaries from this repository
...
| --- | --- | --- | --- | --- |
| Tree-sitter grammar + parser crate | this repo root | Zed grammar loader, Rust parser consumers, future analysis crate | source repository | git commit / pinned `rev` |
| `structurizr-analysis` | future workspace crate in this repo | future `structurizr-lsp` | source-only workspace member | none initially |
| `structurizr-lsp` | future workspace crate in this repo | Zed extension, other editors, manual users | platform binaries / archives | GitHub release tag + assets |
...
- do not commit `structurizr-lsp` binaries into `zed-structurizr`
...
The first release assets for `structurizr-lsp` should stay boring and deterministic.
...
- `structurizr-lsp`
- or `structurizr-lsp.exe` on Windows

Avoid nested release-directory layouts like:

- `structurizr-lsp-v0.4.0-macos-aarch64/structurizr-lsp`
...
## Loop 2: LSP implementation work once the crate exists

Once `crates/structurizr-analysis/` and `crates/structurizr-lsp/` exist, the day-to-day loop should stay mostly in this repository.
...
3. run targeted Rust validation:
   ...
   - LSP crate tests
4. build the local server binary
...
- add release automation for `structurizr-lsp` platform builds once the crate exists
...
- the future LSP can be editor-agnostic at the binary level while still integrating cleanly with Zed
