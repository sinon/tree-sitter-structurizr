...
impl WorkspaceLoadFailure {
    ...
    fn document_read(path: &Path, error: io::Error) -> Self {
        Self::unanchored(
            ...
            format!("failed to read workspace document {}: {error}", path.display()),
            Some(path.to_path_buf()),
        )
    }
...
impl WorkspaceLoader {
    ...
    where
        ...
        P: AsRef<Path>,
    {
        self.load_paths_with_failures(roots).map_err(io::Error::other)
    }
                ...
                self.load_document(resolved_base.path.clone(), false)
                    .map_err(|error| {
                        WorkspaceLoadError::single(WorkspaceLoadFailure::workspace_base(
                            ...
                            format!(
                                "failed to load workspace base {}: {error}",
                                resolved_base.target_text
                            ),
                        ))
                    })?;
                ...
                match event {
                    ...
                    WorkspaceDirectiveEvent::IncludeDirective(index) => {
                        ...
                        for included_path in &resolved_include.discovered_paths {
                            self.load_document(included_path.clone(), false).map_err(|error| {
...
fn resolve_workspace_base(
    ...
    if !is_supported_local_include_path(&relative_target) {
        return Err(base_error(
            Some(base_path),
            format!(
                "workspace base path escapes the allowed subtree: {base_text}"
            ),
        ));
    }
