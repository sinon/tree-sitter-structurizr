...
#[test]
fn ambiguous_element_selector_targets_keep_coarse_reference_diagnostics() {
    let (_workspace, facts) = load_temp_workspace(
        &[
            (
                "workspace.dsl",
    ...
    let diagnostics = diagnostics_of_code(&facts, "semantic.ambiguous-reference");
    assert_eq!(diagnostics.len(), 1);
    assert_eq!(
        diagnostics[0].message(),
        "ambiguous identifier reference: api"
    );
}
