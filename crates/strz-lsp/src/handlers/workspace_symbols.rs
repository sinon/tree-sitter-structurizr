...
pub async fn workspace_symbol(
    ...
    let Some(symbols) = load_workspace_symbol_information(backend, &params.query).await else {
        debug!(
...
async fn ensure_workspace_facts(backend: &Backend) {
    ...
    let workspace_load = super::text_sync::reload_workspace_state(backend, None).await;
...
async fn load_workspace_symbol_information(
    backend: &Backend,
    query: &str,
    ...
    let symbols = {
        ...
        workspace_symbol_information(&state, workspace_facts, query)
    };
    drop(state);
...
fn symbol_information(
    ...
    #[allow(deprecated)]
    // field `deprecated` has been deprecated in favor of using tags and will be removed in the future
