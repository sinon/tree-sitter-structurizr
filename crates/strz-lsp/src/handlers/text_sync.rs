...
pub async fn did_close(backend: &Backend, params: DidCloseTextDocumentParams) {
    ...
    let workspace_load =
        reload_workspace_state_after_close(backend, closed_document.as_ref()).await;
...
async fn publish_latest_snapshot(backend: &Backend, document: DocumentState) {
    ...
    let workspace_load = reload_workspace_state(backend, Some(&document)).await;
...
/// Publishes session-level workspace-load failures without blocking the text-sync
/// handler on extra client notifications.
///
/// We spawn this small task after state writes complete so `didOpen`/`didChange`/
/// `didClose` can finish their main diagnostic pipeline promptly. The task still
/// preserves per-message ordering by sending `showMessage` before `logMessage`
/// for each failure in sequence.
...
/// Reloads workspace facts and fatal load failures for the current open-document
/// set, applying the in-memory text of the actively edited document when present.
pub(super) async fn reload_workspace_state(
    backend: &Backend,
    current_document: Option<&DocumentState>,
) -> WorkspaceLoadState {
    reload_workspace_state_for_event(backend, current_document, current_document).await
}

async fn reload_workspace_state_after_close(
    backend: &Backend,
    closed_document: Option<&DocumentState>,
) -> WorkspaceLoadState {
    reload_workspace_state_for_event(backend, None, closed_document).await
}

/// Reloads workspace state after one text-sync event.
///
/// `current_document` carries the live in-memory text that should override any
/// file-backed content during the load. `event_document` keeps the triggering
/// document available for fallback root discovery when the client has not
/// configured workspace folders, including close events where the document is no
/// longer part of the open-document set.
async fn reload_workspace_state_for_event(
    backend: &Backend,
    current_document: Option<&DocumentState>,
    ...
    let Some(load_paths) = load_paths else {
        debug!(
